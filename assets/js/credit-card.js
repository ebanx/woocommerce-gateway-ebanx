	...
	function($) {
			...
			var wc_ebanx_form = {
				...
				toggleCardUse: function () {
					$( document ).on(
						'click', 'li[class*="payment_method_ebanx-credit-card"] .ebanx-credit-card-label', function () {
			...
			$( document ).on(
				'change', 'select.ebanx-instalments', function (){
					update_converted( $( this ) );
				}
			);

			$( document ).on(
				'change', 'input[name="ebanx-credit-card-use"]', function () {
					update_converted(
						$( this )
