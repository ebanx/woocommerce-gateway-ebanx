/* global wcEbanxParams, EbanxCheckout */
(function( $ ) {
	'use strict';
	...
	$( function() {
		...
		$( 'form.checkout' ).on( 'checkout_place_order_ebanx-credit-card', {
			preserveEbanxCheckoutSubmitValue: true
		}, isCheckoutInvalid );
		...
		function isCheckoutInvalid( evt ) {
			...
			if ( formSubmit ) {
				if ( 'undefined' !== typeof evt && 'undefined' !== typeof evt.data ) {
					if ( 'undefined' !== typeof evt.data.preserveEbanxCheckoutSubmitValue && ! evt.data.preserveEbanxCheckoutSubmitValue ) {
						formSubmit = false;
					}
				}
				return true;
			...
			if ( ! $( '#payment_method_ebanx-credit-card' ).is( ':checked' ) ) {
				return true;
			}

			if ( 0 < $( 'input[name=ebanx_checkout_token]' ).length ) {
				return true;
			}
        ...
        //
		// 	// Create checkout.
		// 	checkout = new EbanxCheckout.Checkout({
		// 		encryption_key: wcEbanxParams.encryptionKey,
		// 		success: function( data ) {
		// 			formSubmit = true;
        //
		// 			// Remove any old token input.
		// 			$( 'input[name=ebanx_checkout_token]', form ).remove();
        //
		// 			// Add the token input.
