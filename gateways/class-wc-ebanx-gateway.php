...
class WC_EBANX_Gateway extends WC_Payment_Gateway {
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		try {
			...
			$order->add_order_note( sprintf( __( 'EBANX: Refund requested. %1$s - Refund ID: %2$s - Reason: %3$s.', 'woocommerce-gateway-ebanx' ), wc_price( $amount ), $request->refund->id, $reason ) );
	...
	public function get_currency_rate( $local_currency_code ) {
		...
		$usd_to_local = \Ebanx\Ebanx::getExchange(
			array(
